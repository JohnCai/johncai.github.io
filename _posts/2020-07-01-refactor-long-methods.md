...
如果花 1 小时的时间重构一下代码，可以节省后续 n 小时的读代码时间，那么，重构不应该成为程序员的基本良心吗？
...
反过来说，若因为加功能或者其它任何原因，需要去读某一段代码，就应该对它进行重构。

你 3 分钟前写的代码，一定还需要去读它吧？那么，你应该重构你 3 分钟前写的代码。

这就是「重构不是吃饭，饿了才吃；重构是呼吸，随时随地」的道理。

### 如何开始重构？

都知道抽方法是最常见的重构手法之一，可是什么时候抽方法呢？

**我建议的重构入门第一准则：所有的 Public 方法，由且只由 Private 方法组成。**

如果上面的`吃饭();` `睡觉();` `打豆豆();` 就是 3 个私有方法，对其适当地命名，让名字清晰表达它的意图。

如此，你不怎么需要写注释。

那些说注释越多越好的，可能，还没学会写代码。

### 生成代码实例
学习 TDD 也好，重构也好，一个很大的障碍，就是没办法把例子里学到的东西应用到生产代码中。

下面是我花了 1 小时重构的现实中原封不动的代码，比上面的傻瓜示例肯定更有挑战。

但是，使用上面的第一准则，已经足够让你开始踏上重构这条路了。
